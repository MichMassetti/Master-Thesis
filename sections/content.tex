...
The vulnerability relies on the shared variable price, which is updated by the function changePrice (\ref{lst:RaceCodition} line 15) and used by the function buy (\ref{lst:RaceCodition} line 2).
\begin{lstlisting} [language={C++},caption={Cross-function RaceCondition vulnerable functions.}, label={lst:RaceCodition}]
...
The snipped code \ref{lst:internalTransferFrom} shows the vulnerable function. 
...
\begin{lstlisting} [language={C++},caption={Vulnerable function in LoanTokenLogicStandard contract.}, label={lst:internalTransferFrom}]
...
The code \ref{lst:CorrectinternalTransferFrom} presents some differences. The operations regarding the receiver's balance are computed (lines 13-15), then those which deal with the sender's one (16-20).
\begin{lstlisting} [language={C++},caption={Corrected bug in LoanTokenLogicStandard contract.}, label={lst:CorrectinternalTransferFrom}]
    function _internalTransferFrom(
        address _from,
        address _to,
        uint256 _value,
...
\begin{lstlisting} [language=C++, caption={Backdoor inside the contract}, label={lst:SBREE}]
...
The tool is implemented as an extension of VeriSmart \ref{sec:WithoutSpecification:VeriSmart}.
...
\end{itemize}
The installation of VeriSmart is necessary for running the tool. After that, the command \ref{lst:SmarTestRun} is run for using VeriSmart in SmarTest mode.
...
The report \ref{lst:SmarTestReport} shows an example of output of SmarTest, which provides the sequence of funtions for exploiting the found bug.
