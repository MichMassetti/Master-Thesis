...
\label{ch:Tools}
argument for the choice of tools.
Chosen tools because more cited in the papers, the one with recent updates and manteind, open source, one representative closed source tool 
Overview of the tools and decision for the classification
In previous knowledge I provide the definition of Symbolic execution Fuzzing Formal Verification and so on.
\section{Tools with multiple mood running}
We considere the tools with the possibility to rub as a scanner without specifying any specification, 
...
The tool Manticore \autoref{sec:Specification:Manticore} has been described in the other section because it is allowed to use in multiple modes. 
For our purpose, we adopted it in both ways, for the detection of vulnerabilities involving malicious external calls.
