...
one subchapt blockchain + smart cotracts
\section{Ethereum}
\label{sec:Backgroud:Ethereum}
Example of blockchain for smart contracts 
EVM compatible a bit of story 
Solidity
...
\begin{itemize}
    \item Fuzzing
    \item Formal Verification (, smt, precondition, postcodition)
    \item Symbolic Execution (it's part of formal verification, but for our point of view, it's better representig like this ganau)
\end{itemize}
...
NoList shows in practical the expressed properties.
Property 1 postcondition refered to function processRefunds():
\begin{equation}
      \_refundProgress<=\_bidIndex
\end{equation}

Property 2 precondition referred claimProjectFunds() : refundProgress >= totalBids
...
We compare the miner.rewardWriteoff with its recumpute mathematical valu with the updated parameters, 
as shown by \autoref{lst:CoverProps}.


\begin{lstlisting} [caption={Cover properties}, label={lst:CoverProps}]
    Postcondition refered to function deposit(...): 
...
\autoref{lst:BZXProps} encompasses those properties, the post-condition should be attached to all the functions involving the function \_internalTransferFrom(...), 
since it is internal; the invariant is referred to all the contract.

\begin{lstlisting} [caption={BZX properties}, label={lst:BZXProps}]
    Postcondition: 
    old_value_of(balances[_from])>=balances[_from]
    && old_value_of(balances[_to])<=balances[_to]
    
    Invariant:  sum_of_uint(balances) <= totalSupply
\end{lstlisting}
...
\begin{lstlisting} [caption={XSURGE properties}, label={lst:XSURGEProps}]
    Invariant: 
    sum_of_uint(_balances) <= _totalSupply
    Postcondition: 
    _totalSupply <=old_value_of(_totalSupply)
\end{lstlisting}
...
as shown by \autoref{lst:SpartanProps}.

\begin{lstlisting} [caption={Spartan properties}, label={lst:SpartanProps}]
    Postcondition: 
...
\_amount.mul(userBonus(\_pid, \_user).add(10000)).div(10000).
...
For the detection of the vulnerability, we specify the property as a postcondition (\autoref{lst:UraniumProps}) of the function withdraw. 


\begin{lstlisting} [caption={Uranium properties}, label={lst:UraniumProps}]
    Postcondition: 
    userInfo[msg.sender].bonus-userInfo[msg.sender].rewardDebt -> userInfo[msg.sender].bonus < old_value_of(userInfo[msg.sender].bonus)
\end{lstlisting}
...
\begin{lstlisting} [caption={BurgerSwap properties}, label={lst:BurgerSwapProps}]
    Postcondition: 
    old_value_of(address(this).balance) == address(this).balance - _amount
\end{lstlisting}
...
\autoref{lst:DirtyDogsProps} states the post-condition, referred to the vulnerable function claimDogs(...).

\begin{lstlisting} [caption={DirtyDogs properties}, label={lst:DirtyDogsProps}]
    Postcondition: 
