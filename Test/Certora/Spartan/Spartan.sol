...
contract Pool {
    ...
    constructor (address base, address tok){
        BASE = base;
        TOKEN = tok;
        decimals = 18;
        balanceBase[msg.sender]=1000;
        baseAmount=1000;
        balanceToken[msg.sender]=1000;
        tokenAmount=1000;
    }
    ...
    function balanceBaseOf (address a) public view returns (uint256){
        return balanceBase[a];
    }
    function balanceTokenOf (address a) public view returns (uint256){
        return balanceToken[a];
    }
    function balanceBaseOfThis () public view returns (uint256){
        return balanceBase[address(this)];
    }
    function balanceTokenOfThis () public view returns (uint256){
        return balanceToken[address(this)];
    }
