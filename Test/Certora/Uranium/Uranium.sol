...
contract UraniumBonusAggregator is Ownable{
    ...
    mapping(address =>  uint16) public userBonusOnFarms;
    ...
    function addUserBonusOnFarm(address _user, uint16 _percent, uint256 _pid) external onlyVerifiedContract{
        ...
        userBonusOnFarms[_user] = uint16(userBonusOnFarms[_user].add(_percent));
        master.updateUserBonus(_user, _pid, userBonusOnFarms[_user]);
    }

    function removeUserBonusOnFarm(address _user, uint16 _percent, uint256 _pid) external onlyVerifiedContract{
        require(_percent < 10000, "Invalid percent");
        userBonusOnFarms[_user] = uint16(userBonusOnFarms[_user].sub(_percent));
        master.updateUserBonus(_user, _pid, userBonusOnFarms[_user]);
    }

    function getBonusOnFarmsForUser(address _user, uint256 _pid) public view returns (uint16){
        return userBonusOnFarms[_user];
    }
...
contract MasterUranium is Ownable {
    ...
    function set(uint256 _allocPoint, uint16 _depositFeeBP, bool _isSRadsRewards, bool _withUpdate) external onlyOwner {
        uint256 prevAllocPoint = poolInfo.allocPoint;
        poolInfo.allocPoint = _allocPoint;
        poolInfo.depositFeeBP = _depositFeeBP;
        poolInfo.isSRadsRewards = _isSRadsRewards;
    ...
    function pendingRads(address _user) external view returns (uint256) {
    ...
    function updateUserBonus(address _user, uint256 bonus) external  onlyAggregator{
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[_user];
        updatePool();
        if (user.amount > 0) {
    ...
    function deposit( uint256 _amount) external {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                uint256 _bonusAmount = _amount.sub(depositFee).mul(userBonus(_user).add(10000)).div(10000);
                user.amountWithBonus = user.amountWithBonus.add(_bonusAmount);
                pool.lpSupply = pool.lpSupply.add(_bonusAmount);
            } else {
                user.amount = user.amount.add(_amount);
                uint256 _bonusAmount = _amount.mul(userBonus( _user).add(10000)).div(10000);
                user.amountWithBonus = user.amountWithBonus.add(_bonusAmount);
                pool.lpSupply = pool.lpSupply.add(_bonusAmount);
            }
        }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if(_amount > 0) {
            ...
            uint256 _bonusAmount = _amount.mul(userBonus(msg.sender).add(10000)).div(10000);
    ...
    returns(address lpToken, uint256 allocPoint, uint256 lastRewardBlock,
        ...
        return (
            ...
            poolInfo.accRadsPerShare,
            poolInfo.depositFeeBP,
            poolInfo.isSRadsRewards
        );
    }
